...
public record PathIndexingConf(@Valid Map<String, PathIndexingRealm> realms,
							   ...
							   String spoolScans,
							   String spoolEvents) {// TODO test

	private static final Pattern REMOVE_SPACES = Pattern.compile("\\s");
	...
	static String correctName(final String rawname, final String fieldName) {
		...
		final var noSpacesName = REMOVE_SPACES.matcher(rawname.trim()).replaceAll("_");
		final var name = normalize(noSpacesName, NFKD).replaceAll("\\p{M}", "")
				.replace(':', '_')
				.replace('/', '_')
				.replace('\\', '_')
				.replace('~', '_')
