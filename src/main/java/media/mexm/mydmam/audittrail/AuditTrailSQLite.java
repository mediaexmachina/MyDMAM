...
public class AuditTrailSQLite { // TODO test
	...
	private void prepareSQLite() throws SQLException {
		...
		try (final var statement = connection.createStatement()) {
			...
			final var createIndex = """
					CREATE INDEX "idx-events-object" ON "events" ("object");
					""";
	...
	public synchronized void insert(final long date,
									final String issuer,
									final String event,
									final Collection<BatchInsert> batchInserts) throws SQLException {
		if (batchInserts.isEmpty()) {
			return;
		}
		checkConnection();
		final var sql = "INSERT INTO events (date, issuer, event, object, payload) VALUES (?, ?, ?, ?, ?)";
		try (final var statement = connection.prepareStatement(sql)) {
			...
			for (final var entry : batchInserts) {
				statement.setString(4, entry.object());
				statement.setString(5, entry.payload());
				statement.addBatch();
			}
